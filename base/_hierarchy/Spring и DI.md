---
tags:
  - category/java
  - system/high/hierarchy
aliases: []
relevant: false
category:
  - "[[java]]"
meta:
problem:
icon: 🧬
color: "#b089c4"
created: 2026-06-22T00:04:26+03:00
updated: 2026-06-22T00:07:17+03:00
---

[[java @Configuration и @Bean|Configuration и @Bean]]
[[java @Profile]]
[[java Autowired на сеттере|@Autowired на сеттере]]
[[java ContextLoadListener|ContextLoadListener]]
[[java PostConstruct и @PreDestroy]]
[[java WebApplicationContext|WebApplicationContext]]
[[java aop в java|aop в java]]
[[java виды AOP|виды AOP]]
[[java внедрение зависимости от другого бина]]
[[java внедрение значений из property-файлов]]
[[java именование бинов|именование бинов]]
[[java интерфейсы InitializingBean и DisposableBean]]
[[java контекст Spring|контекст Spring]]
[[java механизм автоконфигурации Spring Boot]]
[[java плюсы Spring Boot|плюсы Spring Boot]]
[[java пояснения к pom.xml|пояснения к pom.xml]]
[[java пример application.yaml]]
