module.exports = {
  entry: async (params) => {
    ...
    try {
      ...
      const projectTitle = sanitizeFileName(inputTitle);
      if (!projectTitle) {
        new Notice('Project creation cancelled: title is required.');
        return;
      }

      const projectTypes = await loadProjectDefinitions(
        app,
        'templates/create/projects'
      );

      if (projectTypes.length === 0) {
        new Notice('No project templates found.');
        return;
      }

      const shortList = projectTypes.filter(
        (item) => String(item.group || '').toLowerCase() === 'short'
      );
      const initialList = shortList.length > 0 ? shortList : projectTypes;
      ...
      if (selected.__showAll) {
        selected = await quickAddApi.suggester(
          projectTypes.map(formatMenuLabel),
          projectTypes,
          false,
          'Project type (full list):'
        );
      }
      ...
      const targetFolder = selected.folder || 'projects';
      const projectPath = getUniquePathWithExtension(
        app,
        targetFolder,
        projectTitle,
        '.md'
      );
      const projectBaseName = projectPath.split('/').pop().replace(/\.md$/, '');
      ...
      if (!isMarkdownFile(createdFile)) {
        createdFile = app.vault.getAbstractFileByPath(projectPath);
      }
      if (!isMarkdownFile(createdFile)) {
        throw new Error(`Cannot create project note: ${projectPath}`);
      }
      ...
      new Notice(`Projects macro error: ${message}`);
      console.error('Projects macro error', error);
    }
  },
};
...
async function loadProjectDefinitions(app, rootPath) {
  const root = app.vault.getAbstractFileByPath(rootPath);
  if (!isFolder(root)) {
    throw new Error(`Projects root folder not found: ${rootPath}`);
  }
  ...
  for (const child of root.children) {
    ...
    result.push({
      ...
      emoji: icon,
      folder: normalizeManifestString(manifest.folder) || 'projects',
