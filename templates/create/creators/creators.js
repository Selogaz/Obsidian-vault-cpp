module.exports = {
  entry: async (params) => {
    ...
    try {
      ...
      const creatorTitle = sanitizeFileName(inputTitle);
      if (!creatorTitle) {
        new Notice('Creator creation cancelled: title is required.');
        return;
      }

      const creatorTypes = await loadCreatorDefinitions(
        app,
        'templates/create/creators'
      );

      if (creatorTypes.length === 0) {
        new Notice('No creator templates found.');
        return;
      }

      const shortList = creatorTypes.filter(
        (item) => String(item.group || '').toLowerCase() === 'short'
      );
      const initialList = shortList.length > 0 ? shortList : creatorTypes;
      ...
      if (!selected) {
        new Notice('Creator creation cancelled.');
        return;
      }

      if (selected.__showAll) {
        selected = await quickAddApi.suggester(
          creatorTypes.map(formatMenuLabel),
          creatorTypes,
          false,
          'Creator type (full list):'
        );
      }
      ...
      const targetFolder = selected.folder || 'base/creators';
      const creatorPath = getUniquePathWithExtension(
        app,
        targetFolder,
        creatorTitle,
        '.md'
      );
      const creatorBaseName = creatorPath.split('/').pop().replace(/\.md$/, '');
      ...
      if (!isMarkdownFile(createdFile)) {
        createdFile = app.vault.getAbstractFileByPath(creatorPath);
      }
      if (!isMarkdownFile(createdFile)) {
        throw new Error(`Cannot create creator note: ${creatorPath}`);
      }

      const leaf = app.workspace.getLeaf(true);
      await leaf.openFile(createdFile);

      new Notice(`Creator created: ${createdFile.basename}`);
    } catch (error) {
      const message = error?.message || String(error);
      new Notice(`Creators macro error: ${message}`);
      console.error('Creators macro error', error);
    }
  },
};
...
async function loadCreatorDefinitions(app, rootPath) {
  const root = app.vault.getAbstractFileByPath(rootPath);
  if (!isFolder(root)) {
    throw new Error(`Creators root folder not found: ${rootPath}`);
  }
  ...
  for (const child of root.children) {
    ...
    result.push({
      ...
      emoji: icon,
      folder: normalizeManifestString(manifest.folder) || 'base/creators',
