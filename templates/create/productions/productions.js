module.exports = {
  entry: async (params) => {
    ...
    try {
      ...
      const productionTitle = sanitizeFileName(inputTitle);
      if (!productionTitle) {
        new Notice('Production creation cancelled: title is required.');
        return;
      }

      const productionTypes = await loadProductionDefinitions(
        app,
        'templates/create/productions'
      );

      if (productionTypes.length === 0) {
        new Notice('No production templates found.');
        return;
      }

      const shortList = productionTypes.filter(
        (item) => String(item.group || '').toLowerCase() === 'short'
      );
      const initialList = shortList.length > 0 ? shortList : productionTypes;
      ...
      if (!selected) {
        new Notice('Production creation cancelled.');
        return;
      }

      if (selected.__showAll) {
        selected = await quickAddApi.suggester(
          productionTypes.map(formatMenuLabel),
          productionTypes,
          false,
          'Production type (full list):'
        );
      }
      ...
      const targetFolder = selected.folder || 'base/productions';
      const productionPath = getUniquePathWithExtension(
        app,
        targetFolder,
        productionTitle,
        '.md'
      );
      const productionBaseName = productionPath
        .split('/')
        .pop()
        .replace(/\.md$/, '');
      ...
      if (!isMarkdownFile(createdFile)) {
        createdFile = app.vault.getAbstractFileByPath(productionPath);
      }
      if (!isMarkdownFile(createdFile)) {
        throw new Error(`Cannot create production note: ${productionPath}`);
      }

      const leaf = app.workspace.getLeaf(true);
      await leaf.openFile(createdFile);

      new Notice(`Production created: ${createdFile.basename}`);
    } catch (error) {
      const message = error?.message || String(error);
      new Notice(`Productions macro error: ${message}`);
      console.error('Productions macro error', error);
    }
  },
};
...
async function loadProductionDefinitions(app, rootPath) {
  const root = app.vault.getAbstractFileByPath(rootPath);
  if (!isFolder(root)) {
    throw new Error(`Productions root folder not found: ${rootPath}`);
  }
  ...
  for (const child of root.children) {
    ...
    result.push({
      ...
      emoji: icon,
      folder: normalizeManifestString(manifest.folder) || 'base/productions',
