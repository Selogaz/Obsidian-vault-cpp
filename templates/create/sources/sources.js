module.exports = {
  entry: async (params) => {
    ...
    try {
      ...
      const sourceTitle = sanitizeFileName(inputTitle);
      if (!sourceTitle) {
        new Notice('Source creation cancelled: title is required.');
        return;
      }

      const sourceTypes = await loadSourceDefinitions(
        app,
        'templates/create/sources'
      );

      if (sourceTypes.length === 0) {
        new Notice('No source templates found.');
        return;
      }

      const shortList = sourceTypes.filter(
        (item) => String(item.group || '').toLowerCase() === 'short'
      );
      const initialList = shortList.length > 0 ? shortList : sourceTypes;
      ...
      if (!selected) {
        new Notice('Source creation cancelled.');
        return;
      }

      if (selected.__showAll) {
        selected = await quickAddApi.suggester(
          sourceTypes.map(formatMenuLabel),
          sourceTypes,
          false,
      ...
      const targetFolder = selected.folder || 'sources';
      const sourcePath = getUniquePathWithExtension(
        app,
        targetFolder,
        sourceTitle,
        '.md'
      );
      const sourceBaseName = sourcePath.split('/').pop().replace(/\.md$/, '');
      ...
      if (!isMarkdownFile(createdFile)) {
        createdFile = app.vault.getAbstractFileByPath(sourcePath);
      }
      if (!isMarkdownFile(createdFile)) {
        throw new Error(`Cannot create source note: ${sourcePath}`);
      }

      const leaf = app.workspace.getLeaf(true);
      await leaf.openFile(createdFile);

      new Notice(`Source created: ${createdFile.basename}`);
    } catch (error) {
      const message = error?.message || String(error);
      new Notice(`Sources macro error: ${message}`);
      console.error('Sources macro error', error);
    }
  },
};
...
async function loadSourceDefinitions(app, rootPath) {
  const root = app.vault.getAbstractFileByPath(rootPath);
  if (!isFolder(root)) {
    throw new Error(`Sources root folder not found: ${rootPath}`);
  }
  ...
  for (const child of root.children) {
    ...
    result.push({
      ...
      emoji: icon,
      folder: normalizeManifestString(manifest.folder) || 'sources',
