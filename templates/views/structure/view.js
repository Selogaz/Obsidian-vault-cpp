...
const getColors = () => {
  ...
  return {
    HIERARCHY:
      rootStyles.getPropertyValue('--structure-color-hierarchy').trim() ||
      '#3f2759',
    META:
      rootStyles.getPropertyValue('--structure-color-meta').trim() || '#211f73',
    PROBLEM:
      rootStyles.getPropertyValue('--structure-color-problem').trim() ||
      '#4f482f',
    AGGREGATOR_STROKE:
      rootStyles
        .getPropertyValue('--structure-color-aggregator-stroke')
        .trim() || '#db7923',
    IGNORE_STROKE:
      rootStyles.getPropertyValue('--structure-color-ignore-stroke').trim() ||
      '#FF6347',
  };
};
...
const createNodeStyle = (node, fill, hasAggregator, page) =>
  `class ${node} internal-link; style ${node} fill:${fill}${
    hasAggregator
      ? `,stroke:${COLORS.AGGREGATOR_STROKE},stroke-width:6px,stroke-dasharray: 5 5`
      : ''
  }${
    page?.file.tags?.includes('#mark/ignore')
      ? `,stroke:${COLORS.IGNORE_STROKE},stroke-width:2px`
      : ''
  };`;
...
function buildHierarchyStructure() {
  ...
  const mainHierarchies = hierarchies
    .where(
      (p) =>
        !hierarchies.file.link.some((link) => p.file.inlinks.includes(link))
    )
  ...
  function processHierarchy(parentIndex, hierarchy) {
    ...
    outgoingHierarchies.forEach((outHierarchy) => {
      ...
      structure.push(
        `hNode${parentIndex} -->|"${getEdgeLabel(
          'hierarchy'
        )}"| hNode${currentIndex}["${outHierarchy.file.name}"];`,
        createNodeStyle(
          `hNode${currentIndex}`,
          COLORS.HIERARCHY,
  ...
  mainHierarchies.forEach((hierarchy) => {
    const currentIndex = nodeCounter++;
    structure.push(
      `hNode0["🧬 hierarhies"] -->|"${getEdgeLabel(
        'hierarchy'
      )}"| hNode${currentIndex}["${hierarchy.file.name}"];`,
      createNodeStyle(
        `hNode${currentIndex}`,
        COLORS.HIERARCHY,
...
function buildMetaStructure() {
  ...
  const processHierarchy = (parentNode, hierarchy, prefix) => {
    hierarchies
      ...
      .forEach((outHierarchy, i) => {
        ...
        structure.push(
          `${parentNode} -->|"${getEdgeLabel('hierarchy')}"| ${nodeId}["${
            outHierarchy.file.name
          }"]`,
          createNodeStyle(
            nodeId,
            COLORS.HIERARCHY,
  ...
  metaNotes.forEach((meta, i) => {
    ...
    structure.push(
      `mNode0["🔬 meta-notes"] -->|"${getEdgeLabel('meta')}"| ${metaNodeId}["${
        meta.file.name
      }"]`,
      createNodeStyle(
        metaNodeId,
    ...
    if (metasHierarchies.length + problems.length > 0) {
      ...
      metasHierarchies.forEach((hierarchy, k) => {
        ...
        structure.push(
          `${metaNodeId} -->|"${getEdgeLabel(
            'hierarchy'
          )}"| ${hierarchyNodeId}["${hierarchy.file.name}"]`,
          createNodeStyle(
            hierarchyNodeId,
            COLORS.HIERARCHY,
            hierarchy.file.tags.includes('#mark/aggregator'),
      ...
      problems.forEach((problem, k) => {
        ...
        structure.push(
          `${metaNodeId} -->|"${getEdgeLabel('problem')}"| ${problemNodeId}["${
            problem.file.name
          }"]`,
          createNodeStyle(
            problemNodeId,
        ...
        problemsHierarchies.forEach((pH, z) => {
          ...
          structure.push(
            `${problemNodeId} -->|"${getEdgeLabel(
              'hierarchy'
            )}"| ${phNodeId}["${pH.file.name}"]`,
            createNodeStyle(
              phNodeId,
              COLORS.HIERARCHY,
...
const mermaidConfig =
  "%%{init: {'flowchart': {'nodeSpacing': 12, 'rankSpacing': 20, 'padding': 10}, 'themeVariables': {'nodePadding': 10}}}%%\n";
