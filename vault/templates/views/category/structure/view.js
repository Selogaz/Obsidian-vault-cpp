...
const createNodeStyle = (node, fill, hasAggregator) =>
  `class ${node} internal-link; style ${node} fill:${fill}${hasAggregator ? ",stroke:#db7923,stroke-width:6px,stroke-dasharray: 5 5" : ""};`;

const hierarchies = dv
  .pages("#system/high/hierarchy")
  .where(isInCategory)
  .sort((p) => p.file.name)
